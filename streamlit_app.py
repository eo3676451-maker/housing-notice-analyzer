...
def extract_companies_from_table(pdf):
    ...
    # 테이블에서 시공사 못 찾으면 텍스트에서 건설사 이름 직접 검색
    if companies["시공사"] is None:
        for page in pdf.pages:
            text = page.extract_text() or ""
            
            # "사업주체 및 시공사" 섹션이 있는 페이지에서만 검색
...
def extract_schedule_from_table(pdf):
    ...
    # 결과를 리스트로 변환 (순서 유지)
    result = []
    order = ["입주자모집공고일", "특별공급 접수일", "일반공급 1순위", "일반공급 2순위",
             "당첨자 발표일", "서류접수 기간", "계약체결 기간", "정당계약 기간", 
             "서류제출 기간", "계약 기간", "입주예정일", "입주지정기간"]
